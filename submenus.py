...
class SubMenu:
    ...
    def increase_submenu_index(self):
        if self.submenu_index < self.submenu_index_range():
    ...
    def draw_submenu(self, screen, y_pos, color):
        text = self.assets.fonts["large"].render(f'{self.name} NOT IMPLEMENDED, COMING SOON!', True, color)
...
class SpecialMenu(SubMenu):
    ...
    def draw_submenu(
        ...
        max_name_width = max(self.assets.fonts["small"].size(name)[0] for name, _, _ in self.stats)
